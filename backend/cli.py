...
@cli.command("compute-album-similarity")
@click.option("--limit", "-l", type=int, default=None, help="Max albums to process")
@click.option("--force", is_flag=True, help="Recompute even if already cached")
@click.option("--album", "-a", type=str, default=None, help="Single album UUID")
def compute_album_similarity_cmd(limit, force, album):
    """Warm/rebuild the audio-similar albums cache (similar_albums).

    The web UI fills this lazily on first view; this command is for bulk
    pre-warming or a full rebuild after large library/embedding changes.
    Incremental by default — skips albums that already have a cached result.
    """
    ...
    if album:
        ...
        sql = """
            ...
            JOIN embeddings e ON e.track_id = mf.track_id
        """
        if not force:
            sql += """ WHERE NOT EXISTS (
                SELECT 1 FROM similar_albums sa
                WHERE sa.album_id = av.album_id AND sa.source = %(src)s)"""
        targets = [r["id"] for r in db_query(sql, {"src": SOURCE})]
        if limit:
            targets = targets[:limit]

    total = len(targets)
    click.echo(f"Computing similar albums for {total} album(s)...")
    t0 = time.time()
    for i, aid in enumerate(targets, 1):
        try:
            n = len(compute_and_cache(aid))
        except Exception as e:
            click.echo(f"  ✗ {aid}: {e}", err=True)
            logger.exception("compute_and_cache failed for %s", aid)
            continue
        if i % 25 == 0 or i == total:
            rate = i / max(time.time() - t0, 1e-6)
            click.echo(f"  {i}/{total} ({n} neighbours for last, {rate:.1f}/s)")
    click.echo("✅ Done.")
