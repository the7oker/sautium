"""The Host guard (auth_hmac.host_allowed): a closed set of this node's own
...
def test_tunnel_address_bound_at_startup_is_accepted(interfaces):
    ...
    assert auth_hmac.host_allowed(f"{TUNNEL}:18000")
    assert auth_hmac.host_allowed(f"{LAN}:8800")


def test_address_that_comes_up_after_startup_is_learned_once(interfaces):
    assert auth_hmac.host_allowed(f"{LAN}:8800")
    assert interfaces.scans == 1
    assert not auth_hmac.host_allowed(f"{TUNNEL}:18000")

    interfaces.own.append(TUNNEL)
    assert not auth_hmac.host_allowed(f"{TUNNEL}:18000")   # the refusal is still cached
...
def test_unbound_literal_is_refused_and_the_miss_expires(interfaces, monkeypatch):
    ...
    scans = interfaces.scans
    assert not auth_hmac.host_allowed(f"{TUNNEL}:18000")
    assert not auth_hmac.host_allowed(TUNNEL)
...
def test_miss_cache_stays_bounded(interfaces):
    for i in range(auth_hmac._MISS_CAP + 40):
        assert not auth_hmac.host_allowed(f"10.9.{i // 256}.{i % 256}")
...
def test_a_name_is_never_resolved_nor_rescanned(interfaces, monkeypatch):
    ...
    assert not auth_hmac.host_allowed("evil.example.com")
    assert not auth_hmac.host_allowed("evil.example.com:8800")
    assert not auth_hmac.host_allowed("[fd00::1]:8800")
...
def test_loopback_forms_and_no_header(interfaces):
    assert auth_hmac.host_allowed("[::1]:8000")
    assert auth_hmac.host_allowed("localhost:8800")
    assert auth_hmac.host_allowed("127.0.0.1")
    assert auth_hmac.host_allowed("host.docker.internal:8800")
    assert auth_hmac.host_allowed("")
...
def test_operator_configuration_is_honoured(interfaces, monkeypatch):
    ...
    assert auth_hmac.host_allowed("100.64.0.9:8800")
    assert auth_hmac.host_allowed("node.tail1234.ts.net")
    assert not auth_hmac.host_allowed("other.tail1234.ts.net")
