...
| `desktop/backup_task.py` | Launcher "Create backup…" / "Export for a friend…" / "Import from file…": runs that CLI on the backend interpreter (`--progress-json --cancel-on-stdin`), streams events to the progress line, cancels through stdin |
| `backend/share.py` | Share export/import (BACKUP.md Product B): gzip'd JSON lines with a signed trailer, streamed both ways; first-hand sealed records only, structural rows from the seed builders, import through `SyncClient._import_items` (the gate) after a verify pass |
