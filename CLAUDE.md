...
| `desktop/backup_task.py` | Launcher "Create backup…" / "Export for a friend…" / "Import from file…": runs that CLI on the backend interpreter (`--progress-json --cancel-on-stdin`) as one job per kind, streams events to the action's row in Settings & Tools (button → red Cancel, progress beneath) and the progress line, cancels through stdin |
