...
## Phase P0+P1: P2P Preparation (Launcher Bridge + DB Refactoring + Node Identity) - DONE

### Phase P0: Launcher ↔ Backend Bridge

**Новий файл:** `desktop/api_client.py`
- Мінімальний HTTP клієнт на `urllib` (без нових залежностей)
- `BackendAPIClient` з методами `get_stats()`, `get_health()`

**Оновлено:** `desktop/launcher.py`
- Секція Library stats (Tracks/Artists/Albums/Embeddings) в 2x2 grid
- Автоматичне оновлення кожні 60 секунд через background thread
- API client підключається після старту сервісів

### Phase P1: DB Refactoring — Genre/Tag/EmbeddingModel → UUID v5

**Новий файл:** `desktop/migrations/002_uuid_genres_tags_models.sql`
- Дедуплікація case-варіантів (напр. "Blues"/"blues"/"BLUES") — 16 дублів об'єднано
- Конвертація SERIAL → UUID v5 для genres, tags, embedding_models
- Оновлення всіх FK references (track_genres, genre_descriptions, artist_tags, album_tags, embeddings, text_embeddings, lyrics_embeddings)
- Результат: genres 398→382, tags 4999, models 4

**Оновлені файли:**
- `backend/uuid_utils.py` — додано `genre_uuid()`, `tag_uuid()`, `embedding_model_uuid()`
- `backend/models.py` — PK/FK типи Integer → UUID(as_uuid=True)
- `backend/scanner.py` — `get_or_create_genre()` використовує `genre_uuid()`
- `backend/embeddings.py` — `_get_or_create_embedding_model()` використовує `embedding_model_uuid()`
- `backend/text_embeddings.py` — аналогічно
- `backend/lyrics_embeddings.py` — аналогічно
- `backend/lastfm.py` — `_store_artist_tags()` / `_store_album_tags()` використовують `tag_uuid()`
- `backend/normalize_genres.py` — використовує `genre_uuid()`
- `desktop/migrations/001_initial.sql` — оновлена DDL для fresh installs

**Верифікація:**
- Python `uuid.uuid5()` == PostgreSQL `uuid_generate_v5()` для всіх test cases
- Всі FK relationships збережені після міграції
- JOIN queries (track → genre, track → embedding) працюють коректно

### Phase P1: Node Identity (Ed25519)
...
**Оновлені файли:**
- `desktop/config_manager.py` — секція `p2p` в DEFAULT_CONFIG
- `desktop/requirements.txt` — додано `cryptography>=41.0.0`
- `desktop/launcher.py` — автоматична генерація identity при старті
