...
### Reachability roadmap (фази B–E)

Фаза A (токени, мастер-контакт, wake/probe, self-check) — SHIPPED. Далі,
кожна фаза самоцінна й розширює ТОЙ САМИЙ `/api/relay/*` контракт:

- **B: Mailbox для пар друг↔друг.** Сьогодні wake закриває напрямок
  "мастер→гість". Пара, де ОБИДВА за CGNAT, каналу не має взагалі:
  повідомлення існує лише у відправника, досяжного дублера немає (у торентах
  цей випадок маскує реплікація — тут маскувати нічим). Deposit/collect
  E2E-блобів на relay дає повідомленню "третього досяжного піра".
- **C: Carry — push-seeding підписаних записів.** Недосяжна нода виштовхує
  свої **канонізовані** first-hand записи (підписи роблять їх
  самоавтентифікованими — роздавати може будь-хто) на досяжного носія;
  далі звичайна sync-топологія розносить їх сама. Чужі рідкісні артисти
  входять у власний announce-бюджет носія (~300) — жодного "анонсую все за
  K клієнтів", урок announce storm. Не-канон резидуй лишається локальним,
  інакше носії розмножують шум.
- **D: Peer-relays.** Досяжні ноди анонсять `Sautium-cap:relay`; клієнт
  ...
  знаходить його ЗВИЧАЙНИМ `lookup_user`. Захист від чорнодірного
